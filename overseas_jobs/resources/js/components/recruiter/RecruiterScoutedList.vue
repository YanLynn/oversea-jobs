<template>
   <div class="user-main">
    <h2 class="user-head-tit recruiter-primarylight-color">{{$t('recruiterScoutedList.bunnerTitle')}}</h2>   
     <div class="content-wrap">     
            <div class="container p-0">
            <div class="searchform-one">
                <!--advanced search-->  
                    <div class="inner-wrapper mb-10">
                        <div class="form-group row"> 
                            <div class="col-12 col-md-3 col-sm-4  mb-sm-0 mb-3">
                                 <label for="求職者氏名">{{ $t('recruiterScoutedList.management_number')}}</label>
                                 <input type="text" :placeholder="[[$t('recruiterScoutedList.management_number')]]" class="form-control" v-model="filteredData.management_number"/>
                            </div>                       
                            <div class="col-12 col-md-3 col-sm-4  mb-sm-0 mb-3">
                                <label for="求人番号">{{ $t('recruiterScoutedList.jobnumber')}}</label>
                                <input type="text" :placeholder="[[$t('recruiterScoutedList.jobnumber')]]" class="form-control" v-model="filteredData.job_num"/>
                            </div>         
                            <div class="col-12 col-md-3 col-sm-4  mb-sm-0 mb-3">
                                 <label for="求人タイトル">{{ $t('recruiterScoutedList.title')}}</label>
                                 <input type="text" :placeholder="[[$t('recruiterScoutedList.title')]]" class="form-control" v-model="filteredData.job_title"/>
                            </div>   
                        </div>     
                        <div class="form-group row">
                            <div class="col-12 col-md-3 col-sm-4  mb-sm-0 mb-3">
                                    <label for="求職者番号">{{ $t('recruiterScoutedList.jobapplicantnumber')}}</label>
                                    <input type="text" :placeholder="[[$t('recruiterScoutedList.jobapplicantnumber')]]" class="form-control" v-model="filteredData.jobseeker_num"/>
                            </div>  
                                <div class="col-12 col-md-3 col-sm-4  mb-sm-0 mb-3">
                                    <label for="求職者氏名">{{ $t('recruiterScoutedList.jobseekername')}}</label>
                                    <input type="text" :placeholder="[[$t('recruiterScoutedList.jobseekername')]]" class="form-control" v-model="filteredData.jobseeker_name"/>
                            </div>   
                      </div>
                        <div class="row col-12">
                            <label for="スカウト日時">{{ $t('recruiterScoutedList.date_time')}}</label>                        
                        </div>
                      <div class="form-group row">
                            <div class="col-12 col-md-3 col-sm-4 mb-sm-0  mb-5 datepicker-wrapper">      
                                <date-picker v-model="filteredData.from_date" valueType="format" class="datepicker" :lang="lang" :placeholder="$t('recruiterScoutedList.date')" ></date-picker>
                            </div>         
                            <div class="col-12 col-md-3 col-sm-4 mb-sm-0 mb-3 datepicker-wrapper similarto position-relative">                      
                                <date-picker valueType="format" class="datepicker" :lang="lang" :placeholder="$t('recruiterScoutedList.date')" v-model="filteredData.to_date"></date-picker>
                            </div> 
                            <div class="col-12 col-md-6 col-sm-4 text-right">
                                <span class="btn primary-btn border-style recruiter-primary-color btn-m" @click="filterFun">{{$t('common.search')}}</span>
                            </div> 
                      </div>
                    </div>               
                    <div class="form-group row col-12">
                         <div class="row col-12"><label for="ステータス">{{ $t('recruiterScoutedList.status')}}</label>
                         <!--status click-->                    
                            ...
                            <span class="tooltip-box" v-if="visible">
                                ...
                                <tbody>
                                    <tr><td class="w-105">回答待ち</td><td>求職者会員からスカウトに対する回答待ち。</td></tr>
                        ...
                        <div class="d-sm-flex">
                        <div class="mr-5 mb-sm-0 mb-2" v-for="status in statusArray" :key="status.id">                        
                            <input
                                v-model="filteredData.scout_status"
                                type="checkbox"
                                :id="status.id"
                                :value="status.id"
                                @change="filterFun()"
                            ...
                            ><div v-if="status.id === '回答待ち'">
                                 {{$t('recruiterScoutedList.waiting_answer')}}  
                            </div>
                            <div v-else-if="status.id === '興味あり'">
                                 {{$t('recruiterScoutedList.interested')}}
                            </div>
                            <div v-else-if="status.id === '不採用/辞退'">
                                 {{$t('recruiterScoutedList.rejected')}}
                            </div>
                            <!-- <div v-else-if="status.id === '期限切れ'">
                                 {{$t('recruiterScoutedList.expired')}}
                            </div> -->
                            <div v-else-if="status.id === '内定'">
                                 {{$t('recruiterScoutedList.unofficial')}}
                            </div>             
                           </label>
                        </div>                            
                        </div>         
           ...
           <div class="row">  
                    <!-- <div class="col-md-5 mb-2">
                        <span class="btn primary-btn border-style delete-color w-50" @click="deleteData()">{{ $t('common.alldelete')}}</span>
                    </div> -->
                    <div class="col-md-12 text-right align-self-lg-center">
                        <span class="text-label-color">{{ $t('recruiterScoutedList.result')}} :<span class="ml-1">{{projects.length}}</span> {{ $t('recruiterScoutedList.total')}}</span>
                        </div>
                    <div class="col-12 custom-table">
                        <div class="pc">
                          
                            <DataTable ref="datatable" :columns="$t('recruiterScoutedList.columns') " :showCheckbox="false" :sortOrders="sortOrders"
                                    @check-all="selectAll" >
                                <tbody>
                                    <tr v-for="project in projects" :key="project.scout_id">
                                        <td class="tbl-ws text-center">{{project.scouted_date}}</td>
                                        <td class="tbl-wxs text-center recruiter-primary-color-txt cursor-pointer"><span @click="showJobDetail(project,true,false)">{{project.management_number}}</span></td>
                                        ...
                                        <td class="recruiter-primary-color-txt tbl-wm cursor-pointer"><span @click="showJobDetail(project)"><span class="txt-vertical-ellipsis">{{project.title}}</span></span></td>
                                        <td class="tbl-wxl recruiter-primary-color-txt text-center cursor-pointer tbl-1024">
                                            <span class="d-flex"  @click="navigateToSeekerDetail(project.scout_id)">
                                                {{project.jobseeker_number}}                                    
                                            <span class="ml-xl-2 txt-vertical-ellipsis-1 w-100 text-left"> {{project.jobseeker_name}}</span>
                                            </span>
                                        </td>                                    
                                        <td class="tbl-status text-center">{{project.scout_status_replace}}</td>
                                        <td class="tbl-ws text-center">
                                        <span class="btn btn-sm table-btn shadow-sm" v-if="allowChat(project.scout_status)" @click="startChat(project)">{{$t('common.chat')}}</span>
                                        ...
                                        <span class="btn btn-sm table-btn shadow-sm" v-if="unclaimed(project.scout_status_replace, project.scout_status) && project.registered_place_type == 'local'" @click="downloadInvoice(project)">{{$t('common.invoice_display')}}</span>
                                        <span class="btn btn-sm table-btn shadow-sm" v-if="unclaimed(project.scout_status_replace, project.scout_status) && project.registered_place_type == 'oversea'" @click="creditPay(project.scout_id)">{{$t('common.kureka_payment')}}</span>
                                        <!-- <span class="btn btn-sm table-btn shadow-sm"  v-if="allowDownloadInvoice(project.job_apply_status)">{{ $t('jobapplicant.display_invoice') }}</span> -->
                                        </td>
                                    </tr>                                
                                </tbody>
                            </DataTable> 
                         ...
                         <div class="sp">
                            <DataTable ref="datatable" :columns="$t('recruiterScoutedList.titles') " :showCheckbox="false" :sortOrders="sortOrders"
                                    @check-all="selectAll" >
                                <tbody>
                                    ...
                                    <span class="num-color">({{project.management_number}})</span>
                                    ...
                                    <ul class="tbl-data" :id="project.id" v-bind:class="{'tbl-expand': (current === index) && (status == true)}">
                                        <li class="toggle-list">
                                            ...
                                            <div class="list-r">
                                                <span class="recruiter-primary-color-txt" @click="showJobDetail(project,true,false)">{{project.management_number}}</span>
                                            </div>
                                        </li>
                                        <li class="toggle-list"> 
                                            <div class="list-l">
                                            ...
                                            <div class="list-r">
                                                {{project.scouted_date}}
                                            </div>                                        
                                        </li>
                                        <li class="toggle-list">
                                            <div class="list-l">求人番号</div>
                                            <div class="list-r">
                                                <span class="recruiter-primary-color-txt" @click="showJobDetail(project)">{{project.job_number}}</span>
                                            </div>
                                        </li>
                                        <li class="toggle-list">
                                            <div class="list-l">
                                            ...
                                            <div class="list-r">
                                                <p class="d-flex mb-0">
                                                    <span class="d-flex recruiter-primary-color-txt"  @click="navigateToSeekerDetail(project.scout_id)">
                                    ...
                                    <div class="successful-block">
                                        ...
                                        <div class="successful-wrapper" v-else>              
                                            <dl class="detail-list">
                                                <dt class="detail-head-group text-label-color">管理番号</dt>
                                                <dd class="detail-data-group">{{scout_info["management_number"]}}</dd>
                                            </dl>
                                            <dl class="detail-list">
        ...
        <!--end for table-->
		<transition name="fade">
			<DetailsInfoPopup v-if="showJobDetailsPopup" :show="showJobDetailsPopup" @closed="PopupClosed" :job="selectedJob" :types="employment_types" :show-as-scout-entry="selectedJob.showAsScoutEntry" :show-job="selectedJob.showJob"></DetailsInfoPopup>
		</transition> 
...
export default {
    ...
    data(){
        ...
        return{
            ...
            show: false,
            scoutedId: 0,
            base_url: "/recruiter/scouted-list",
            ...
            /** Control variable for show/hide job detail dialog */
			showJobDetailsPopup: false,
			/** The `job` object that is bind to the child component  */
			selectedJob: {},
            employment_types: [],
            visible:false,
            current:null,
    ...
    methods: {
        ...
        allowChat(status) {   
            return (status == this.$configs.scouts.interested || status == this.$configs.scouts.unclaimed || status == this.$configs.scouts.billed || status == this.$configs.scouts.payment_confirmed);
        },
        noticeAccept(proj) {   
            return (proj.scout_status == this.$configs.scouts.interested && proj.send_noti == 0);
        },
        unclaimed(status, realStatus){   
            return (this.$configs.scouts.unclaimed.includes(status) && (this.$configs.scouts.billed == realStatus));
        },    
        showNotice(p) {
            this.notice_data.type = 'scout';
            this.notice_data.id = p.scout_id;
        ...
        creditPay(scoutId) {
            this.scoutedId = scoutId;
            this.$api.get(`/recruiter/transaction-list/${this.scoutedId}/scout/transaction-info`)
            .then((r) => {     
                this.scout_info = r.data.info;
        ...
        confirmChoice(ch) {
            let loader = this.$loading.show();
            this.scout_info.choice = ch;
            // this.scout_info.choice = this.card_choice;
            this.$api.post(`/recruiter/transaction-list/credit-payment`, this.scout_info)
            .then((res) => {
        ...
        navigateToSeekerDetail(scoutId) {
			this.$router.push({name: 'JobSeekerInfo', params: { transactionId: Number(scoutId), type: 'scout' }});
		},

        showJobDetail(project,showAsScoutEntry = false, showJob = true) {
            this.selectedJob = {
                ...
                scout_occupation_description : project.scout_occupation_description,
                employment_types: project.employment_types,
                scout_employment_types : project.scout_employment_types,
                job_description: project.job_description,
                qualification: project.qualification,
                job_location: project.job_location,
                scout_job_location: project.scout_job_location,
        ...
        downloadInvoice(project) {
			const loading = this.$loading.show({
				isFullPage: false,
				container: this.$refs.progressContainer,
			});
			this.$api.post(`/recruiter/scouted-list/${project.scout_id}/invoice`, {}, { responseType: "arraybuffer" })
			.then((r) => {
		...
		startChat(scout) {
			const payload = {
				...
				scoutid_or_applyid: scout.scout_id,
				type: 'scout',
